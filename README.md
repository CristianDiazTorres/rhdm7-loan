Red Hat Decision Manager Loan
=============================================
This project will provide you with an example of creating, deploying and leveraging a set of rules
...
The following software is required to run this:
...
1. Add the product ZIP files to the installs directory.

2. Run the `init.sh` (Linux/macOS) or `init.ps1` (Windows) file.

3. Start Red Hat Decision Manager by running `./target/jboss-eap-7.1/bin/standalone.sh` (Linux/macOS) or `.\targer\jboss-eap-7.1\bin\standalone.ps1` (Windows).

4. Login to http://localhost:8080/decision-central
...
1. Click on the "loan-application" project to open the Loan Application project.

2. The project has simple some data model (Loan, Applicant, Reservation), a single decision table (loan-application) which contains the loan approval rule set and a DMN model (reservation) that provides a recomendation based on the credit score.

3. Build and deploy version 1.2.0 of the project. Click on the "Build and Deploy" in the upper right corner.

4. Go to "Menu -> Deploy -> Execution Servers" repository to see the loan-application_1.2.0 KIE Container deployed on the Decision Server.
...
-----------------------------------------
This can be installed on Red Hat OpenShift in various ways. We'll explain the different options provided.
...
This installation option will install the Decision Manager 7 and Decision Service in OpenShift using a single script, after which the project needs to be manually imported.

1. Run the `init-openshift.sh` (Linux/macOS) or `init-openshift.ps1` (Windows) file. This will create a new project and application in OpenShift.

2. Login to your OpenShift console. For a local OpenShift installation this is usually: https://{host}:8443/console

3. Open the project "RHDM7 Loan Demo". Open the "Overview". Wait until the 2 pods, "rhdm7-loan-rhdmcentr" and "rhdm7-loan-kieserver" have been deployed.
...
5. Login to Decision Central:

    ```
    - login for admin and analyst roles (u:dmAdmin / p:redhatdm1!)
    ```
6. Click on "Design" to open the design perspective.

7. Click on "Import project". Enter the following as the repository URL: https://github.com/jbossdemocentral/rhdm7-loan-demo-repo.git , and click on "Import".

8. Select "loan-application" and click on the "Ok" button on the right-hand side of the screen.

9. The project has simple data model (Loan & Applicant) and single decision table (loan-application) which contains the loan approval rule set.

10. Build and deploy version 1.1.0 of the project. Click on the "Build and Deploy" in the upper right corner.

11. Go to "Menu -> Deploy -> Execution Servers" repository to see the loan-application_1.1.0 KIE Container deployed on the Decision Server.

12. The Decision Server provides a Swagger UI that documents the full RESTful interface exposed by the server at. To open the Swagger UI, go back to
...
13. Follow instructions from above "Option 1- Install on your machine", starting at step 11.
...
1. In the demo directory, go to `./support/openshift`. In that directory you will find a `provision.sh` script. (Windows support will be introduced at a later time).

2. Run `./provision.sh -h` to inspect the installation options.

3. To provision the demo, with the OpenShift ImageStreams in the project's namespace, run `./provision.sh setup rhdm7-loan --with-imagestreams`.
